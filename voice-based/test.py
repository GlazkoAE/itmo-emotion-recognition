...
if __name__ == '__main__':
    ...
    validation_loader = torch.utils.data.DataLoader(valid_set, batch_size=4, shuffle=True, num_workers=2)
    ...
    model.load_state_dict(torch.load('my_net_only_all_dataset_resnet_50_pretrained.pth'))

        # utils.overall_accuracy(validation_loader, model)  # выводит общую точность
    utils.metric_for_each_class(validation_loader, model, classes)  # выводит точность по классам
